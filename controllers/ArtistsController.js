...
class ArtistsController {
    ...
    static async getArtists(req, res) {
        try {
            ...
            const { rows } = await dbClient.client.query(query);
            res.json(rows);   
    ...
    static async getArtist(req, res) {
        try {
            ...
            const { rows } = await dbClient.client.query(query, [req.params.id]);
            if (rows.length > 0) {
                res.json(rows[0]);
            } else {
    ...
    static async addArtist(req, res) {
        try {
            ...
            await dbClient.client.query(query, values);
    ...
    static async deleteArtist(req, res) {
        try {
            ...
            await dbClient.client.query(query, [req.params.id]);
